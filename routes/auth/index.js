...
router.all('/logout', middlewares.session.checkLogin(true), async (req, res) => {
    try {
        ...
        // await controllers.userController.updateUserProfile(userId, { status: 'Offline' });
        // req.session.status = 'Offline';

        // if (req.session.show_online) {
            // console.log("req.session.show_online", req.session.show_online);
            // console.log("session status", req.session.status);
